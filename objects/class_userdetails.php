...
class laundry_userdetails {
    ...
    public function reschedule_booking($finaldate,$orderid,$bookingstatus,$readstatus,$lastmodify){
         $query ="UPDATE `ld_bookings` SET `booking_pickup_date_time_start` = '".$finaldate."',`booking_status` = '".$bookingstatus."',`read_status` = '".$readstatus."',`lastmodify` = '".$lastmodify."' where `order_id` = $orderid";
        $result=mysqli_query($this->conn,$query);
        return $result;
    }
