...
class laundry_staff_commision{		
	...
	public function staff_service_details($staff_id){
		$arr = array();
		$query = "select `order_id`, `service_id`, `booking_pickup_date_time_start`, `staff_ids` from `ld_bookings` group by `order_id`, `service_id`, `booking_pickup_date_time_start`, `staff_ids` ORDER BY `order_id` DESC";
	...
	public function get_booking_assign($staff_id){
		$query = "select * from ld_staff_commission where staff_id='".$staff_id."'";
		$result=mysqli_query($this->conn,$query);
		return $result;
	}
