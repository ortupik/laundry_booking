...
if (isset($_POST['btnCreate'])) {
    if (filter_var($_POST['rgMethod'], FILTER_SANITIZE_STRING) == 'numbers') {
        $createResponse = $textLocal->createContacts(filter_var($_POST['txtNumbers'], $_POST['slGroup'], FILTER_SANITIZE_STRING));
    } elseif (filter_var($_POST['rgMethod'], FILTER_SANITIZE_STRING) == 'group') {

        $contacts = array();

        // Build the contacts array
        for ($i = 0; $i < sizeof(filter_var($_POST['txtNumber'], FILTER_SANITIZE_STRING)); $i++) {

            // Skip any empty rows
            if (!is_numeric(filter_var($_POST['txtNumber'][$i], FILTER_SANITIZE_STRING)))
                continue;

            // $contacts[] = new Contact(number, firstname, lastname, custom1, custom2, custom3);
            $contacts[] = new Contact(filter_var($_POST['txtNumber'][$i], $_POST['txtFirstname'][$i], $_POST['txtLastname'][$i], $_POST['txtCustom1'][$i], $_POST['txtCustom2'][$i], $_POST['txtCustom3'][$i], FILTER_SANITIZE_STRING));
        }

        $createResponse = $textLocal->createContactsBulk($contacts, filter_var($_POST['slGroup'], FILTER_SANITIZE_STRING));
    }
            ...
            <div class="fieldRow">
                ...
                <select name="slGroup">
                    ...
                    foreach ($groupResponse->groups as $group) {
                        ?>
                        <option value="<?php echo filter_var($group->id, FILTER_SANITIZE_STRING); ?>"><?php echo filter_var($group->name, FILTER_SANITIZE_STRING); ?></option>
                    <?php 
                    }
                    ?>
                </select>
