...
jQuery(document).ready(function(){
	...
	jQuery(document).on('click','.database_check_con',function(){
		...
		if (!jQuery('#ld_db_form').valid()) {
			...
			var host,dbname,uname,passwords;
			...
			jQuery.ajax({
				...
				success : function(res){
					...
					if(res.indexOf("product successfully verified now!") > -1){
	...
	jQuery(document).on('click','.admin_credential_next',function(){
		jQuery('#ld_admin_detail_form').validate();

		jQuery("#ld_admin_email").rules("add",
		{
			required: true,
			messages: {required: "Please Set Your Username"}
		});
		jQuery("#ld_admin_password").rules("add",
		{
			required: true,
			minlength : 8,
			maxlength : 15,
		...
		if (!jQuery('#ld_admin_detail_form').valid()) {
			return false;
		}
		jQuery.ajax({
			type : 'post',
