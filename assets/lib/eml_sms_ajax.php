...
if(isset($_POST['emlsend'])){
	if($setting->get_option('ld_client_email_notification_status') == 'Y'){	
		...
		die();}
	$cusids=filter_var($_POST['cusids'], FILTER_SANITIZE_STRING);
	$elems = json_decode(filter_var($_POST['cusids'], FILTER_SANITIZE_STRING), true);
	...
	$emlsms->cus_sub=filter_var($_POST['cussub'], FILTER_SANITIZE_STRING);
	$emlsms->cus_msg=filter_var($_POST['cusmsg'], FILTER_SANITIZE_STRING);
...
if(isset($_POST['smssend'])){
	...
	$emlsms->cus_msg=filter_var($_POST['cusmsg'], FILTER_SANITIZE_STRING);
	$emlsms->cus_dtfmt=$dftdt;
	$message=filter_var($_POST['cusmsg'], FILTER_SANITIZE_STRING);
	$elems = json_decode(filter_var($_POST['cusids'], FILTER_SANITIZE_STRING), true);
	$cusids=implode(",", $elems);
	$emlsms->cus_ids=$cusids;
	...
	/*PLIVO CODE*/
		...
		if($chkmsgsnd == "1"){
			echo filter_var("Noo", FILTER_SANITIZE_STRING);
			die();
		}
...
if (isset($_POST['action']) && filter_var($_POST['action'], FILTER_SANITIZE_STRING) == 'check_admin_cus_email')
{
