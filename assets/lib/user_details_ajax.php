...
if(isset($_POST['updatepass'])){
    $objuserdetails->firstname = filter_var($_POST['firstname'], FILTER_SANITIZE_STRING);
    $objuserdetails->lastname = filter_var($_POST['lastname'], FILTER_SANITIZE_STRING);
    $objuserdetails->address = filter_var($_POST['address'], FILTER_SANITIZE_STRING);
    $objuserdetails->city = filter_var($_POST['city'], FILTER_SANITIZE_STRING);
    $objuserdetails->zip = filter_var($_POST['zip'], FILTER_SANITIZE_STRING);
    $objuserdetails->state = filter_var($_POST['state'], FILTER_SANITIZE_STRING);
    $objuserdetails->phone = filter_var($_POST['phone'], FILTER_SANITIZE_STRING);
    $objuserdetails->id = filter_var($_POST['id'], FILTER_SANITIZE_STRING);
    ...
    if (filter_var($_POST['oldpassword'], FILTER_SANITIZE_STRING) != "") {
        if ($op != $dp) {
            $operation = 2;
            echo filter_var("Your Old Password Incorrect...", FILTER_SANITIZE_STRING);
        }
        else {
            $operation = 3;
            if ($np == $rp) {
            ...
            foreach($time_schedule['slots']  as $slot) {
                ...
                if( in_array($complete_time_slot,$time_schedule['booked']) && ($setting->get_option('ld_allow_multiple_booking_for_same_timeslot_status')!='Y') ) { ?>
                    ...
                    if($setting->get_option('ld_hide_faded_already_booked_time_slots')=="on"){
                        ?>
                        <option value="<?php echo date("H:i",strtotime($slot));?>" class="time-slot br-2 ld-booked" >
                            <?php  
                ...
                <?php  
                    ...
                    ?>
                    <option value="<?php echo date("H:i",strtotime($slot));?>" class="time-slot br-2 <?php   if(in_array($complete_time_slot,$time_schedule['booked'])){ echo' ld-booked';}else{ echo filter_var(' time_slotss', FILTER_SANITIZE_STRING); }?>" <?php   if(in_array($complete_time_slot,$time_schedule['booked'])){echo filter_var('', FILTER_SANITIZE_STRING); }else{ echo 'data-slot_date_to_display="'.date($date_format,strtotime($_POST["selected_dates"])).'" data-slot_date="'.$_POST["selected_dates"].'" data-slot_time="'.$slot_time.'"'; } ?>><?php if($setting->get_option('ld_time_format')==24){echo date("H:i",strtotime($slot));}else{echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($slot)));}?></option>
                <?php  
                } $slot_counter++;
            }
...
if(isset($_POST['reschedulebooking'])){
    $id=$order = filter_var($_POST['orderid'], FILTER_SANITIZE_STRING);
    $notes = filter_var($_POST['notes'], FILTER_SANITIZE_STRING);
    $dates = filter_var($_POST['dates'], FILTER_SANITIZE_STRING);
    $timess = filter_var($_POST['timess'], FILTER_SANITIZE_STRING);
    $booking_status = "RS";
    $read_status = "U";
	...
	$client_phone_length = strlen($client_phone_no);
		...
		if($gc_hook->gc_purchase_status() == 'exist'){
			if($setting->get_option('ld_gc_status_configure') == 'Y' && $setting->get_option('ld_gc_status') == 'Y') {
				echo filter_var($gc_hook->gc_reschedule_booking_ajax_hook(), FILTER_SANITIZE_STRING);
			}
		}
        ...
        $subject = "";
        if(filter_var($_POST['user'], FILTER_SANITIZE_STRING) == "customer"){
    ...
    /*SMS SENDING CODE END*/
        ...
        echo filter_var("2", FILTER_SANITIZE_STRING);
    }
}
if(isset($_POST['update_booking_users'])){
  $id=$order = filter_var($_POST['id'], FILTER_SANITIZE_STRING);
	...
	$lastmodify = date('Y-m-d H:i:s');
    $cancel_reson_book = filter_var($_POST['cancel_reson_book'], FILTER_SANITIZE_STRING);
...
if(isset($_POST['insert_crm_user_detail'])){
	/** new customer add **/
	$user->user_pwd=md5(filter_var($_POST['admin_cus_pwd'], FILTER_SANITIZE_STRING));
	$pass=filter_var($_POST['admin_cus_pwd'], FILTER_SANITIZE_STRING);
	$user->first_name=ucfirst(filter_var($_POST['admin_cus_fstnm'], FILTER_SANITIZE_STRING));
	$fnm=filter_var($_POST['admin_cus_fstnm'], FILTER_SANITIZE_STRING);
	$user->last_name=ucfirst(filter_var($_POST['admin_cus_lstnm'], FILTER_SANITIZE_STRING));
	$lnm=filter_var($_POST['admin_cus_lstnm'], FILTER_SANITIZE_STRING);
	...
	$user->phone=filter_var($_POST['admin_cus_phno'], FILTER_SANITIZE_STRING);
	$client_phone = filter_var($_POST['admin_cus_phno'], FILTER_SANITIZE_STRING);
	$user->address=filter_var($_POST['admin_cus_str_addr'], FILTER_SANITIZE_STRING);
	$user->zip=filter_var($_POST['admin_cus_zipcode'], FILTER_SANITIZE_STRING);
	$user->city=filter_var($_POST['admin_cus_city'], FILTER_SANITIZE_STRING);
	$user->state=filter_var($_POST['admin_cus_state'], FILTER_SANITIZE_STRING);
	$user->notes=filter_var($_POST['admin_cus_note'], FILTER_SANITIZE_STRING);
	$user->status='E';
	$user->contact_status="";
	$user->usertype=serialize(array('client'));
	$add_user=$user->add_user();
	if($add_user){
		echo filter_var("Okkk", FILTER_SANITIZE_STRING);
		die();
	}else{
		echo filter_var("NoData", FILTER_SANITIZE_STRING);
		die();
	}
}
...
if(isset($_POST['getallcus'])){
	$emlsms->eml_sms_id=filter_var($_POST['eml_id'], FILTER_SANITIZE_STRING);
	...
	echo filter_var($cus_all_data, FILTER_SANITIZE_STRING);
}
