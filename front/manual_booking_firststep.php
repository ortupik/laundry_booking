...
if (isset($_POST['add_to_cart']))
{
	...
	{
		if($_SESSION['service_id'] != filter_var($_POST['service_id'], FILTER_SANITIZE_STRING))
		{
	...
	if ($_POST['unit_qty'] <= 0 && count($_SESSION['ld_cart']) > 0) {
		for ($i = 0; $i < (count($_SESSION['ld_cart'])); $i++) {
			if ($_SESSION['ld_cart'][$i]['units_id'] == $_POST['units_id'] && $_SESSION['ld_cart'][$i]['service_id'] == $_POST['service_id']) {
		...
		$_SESSION['ld_cart'] = array_values($_SESSION['ld_cart']);
			...
			$cart_details['service_id'] = filter_var($_POST['service_id'], FILTER_SANITIZE_STRING);
